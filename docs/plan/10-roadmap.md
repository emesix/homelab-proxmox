...
- Use a simple flat network, e.g. `10.10.0.0/16`, routed by the *new* OPNsense.
- Assign IPs **already in their future /24 blocks**, e.g.:
  - `10.10.30.x`  → future `LAN-USER` (VLAN 30).
  - `10.10.20.x`  → future `LAN-IOT` (VLAN 20).
  - `10.10.90.x`  → future `LAN-VVE-CCTV` (VLAN 90).
  - `10.10.100–130.x` → future backend / storage / AI ranges.
  - `10.10.200.x` → future `MGMT` (VLAN 200).
...
- Each host has:
  - management IP in the future `10.10.200.0/24` block,
...
- Keep the staging network addressing (`10.10.0.0/16`) during the cutover.
- Ensure:
...
- Convert from `/16` to multiple `/24`s:
  - `VLAN 30` → `10.10.30.0/24` – primary user LAN.
  - `VLAN 20` → `10.10.20.0/24` – IoT (cameras, TV boxes, etc.).
  - `VLAN 90` → `10.10.90.0/24` – VvE CCTV + recorder.
